...
*** Delete all orphaned branches (those that no longer exist remotely)

Once you delete a branch from the remote repository, you can *prune
remote-tracking branches that no longer exist on the remote* using:

: git remote prune $(git origin)

Use the ~--dry-run~ flag to preview which branches will be pruned without actually
pruning them:
...
#+begin_tip
If you want this to be executed automatically every time you fetch/pull:

: git config --global fetch.prune true
#+end_tip

However, note that this command *does not remove local branches* that are already
checked out or that you've created locally.

*Remove orphaned branches in your local repository* that *no longer exist remotely*
(by deleting any local branches that are marked as "gone"):
