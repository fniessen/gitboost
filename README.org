...
directories that are not under Git tracking (e.g., work-in-progress files or
configuration files not committed to the repository). Be cautious as *deleted
files cannot be recovered* unless you have a backup.
...
#+begin_src conf
    ...
    ##?
    ##? Show what would be deleted by git clean-all.
...
: feature-start = '!CURRENT_BRANCH=$1; git checkout master; git pull; git create-branch "$CURRENT_BRANCH" master'

Update the feature branch:

: feature-pull = '!CURRENT_BRANCH=$(git current-branch); git checkout master; git pull; git checkout "$CURRENT_BRANCH"; git rebase master'

Share the feature branch:

: feature-push = '!CURRENT_BRANCH=$(git current-branch); git push -u $(git origin) "$CURRENT_BRANCH"'
