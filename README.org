...
Your =.gitconfig= file might look like the following configuration:
...
If you need to change the user email for client projects, you can use an
~includeIf~ directive to apply a common =.gitconfig= for /all projects located under/
=~/1-Clients/=.

For work projects, create a =.gitconfig= file in the projects' directory to
specify the configuration for those projects:
