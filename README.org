...
git diff --no-index \
    --word-diff=plain \
    --word-diff-regex='[^,]+' \
    <(head -n 6 "$CSV1" | sed 's/,/, /g') \
    <(head -n 6 "$CSV2" | sed 's/,/, /g')
#+end_src
...
#+begin_src conf :tangle no
    ...
    h = "!f() { \
        ...
        if [ -z \"$alias_comments\" ]; then \
            printf >&2 'ERROR: No comments found for the alias\n'; \
            return 2; \
        fi; \
...
Consider using
: git symbolic-ref --short refs/remotes/$(git origin)/HEAD 2> /dev/null | sed 's|^refs/heads/||'
?
...
#+begin_src conf
    ...
    grep-local-branches-in-extension = "!f() { \
        if [ $# -ne 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git grep-local-branches-in-extension EXTENSION PATTERN\n'; \
            return 64; \
        fi; \
        EXTENSION=\"$1\"; \
        PATTERN=\"$2\"; \
...
#+begin_src conf
    ...
    file-last-commit-all-branches = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git file-last-commit-all-branches FILE_PATH\n'; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
...
#+begin_src conf
    file-last-commit-all-local-branches-2 = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git file-last-commit-all-local-branches-2 FILE_PATH\n'; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
...
#+begin_src conf
    ...
    difftool-file-parent = "!f() { \
        [ $# -eq 2 ] || { printf >&2 'Usage: git difftool-file-parent COMMIT FILE_PATH\\n'; exit 64; }; \
        COMMIT=\"$1\"; \
        FILE_PATH=\"$2\"; \
...
#+begin_src conf
    ...
    difftool-file-last = "!f() { \
        [ $# -ge 1 ] || { printf >&2 'Usage: git difftool-file-last FILE_PATH [-- <options>]\\n'; exit 64; }; \
        FILE_PATH=$1; \
        LAST_COMMIT=$(git log -1 --format=%H -- \"$FILE_PATH\") || exit 1; \
        if [ -z \"$LAST_COMMIT\" ]; then \
            printf >&2 'Error: No commits found for %s\\n' \"$FILE_PATH\"; \
            exit 1; \
        fi; \
...
#+begin_src conf
    ...
    branches-containing = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git branches-containing COMMIT\n'; \
            return 64; \
        fi; \
        COMMIT=\"$1\"; \
...
#+begin_src conf
    ...
    tags-containing = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git tags-containing COMMIT\n'; \
            return 64; \
        fi; \
        COMMIT=\"$1\"; \
...
#+begin_src conf
    ...
    grep-string-in-all-commit-snapshots = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git grep-string-in-all-commit-snapshots SEARCH_TERM\n'; \
            return 64; \
        fi; \
        SEARCH_TERM=\"$1\"; \
        git rev-list --all | \
        xargs -n1 sh -c 'git grep -F \"$0\" \"$1\" | sed \"s/^$1:/$(git rev-parse --short $1):/\"' \"$SEARCH_TERM\"; \
    }; f"
#+end_src
...
#+begin_src conf
    ...
    publish-branch = "!f() { \
        if [ $# -ne 0 ]; then \
            printf >&2 'Usage: git publish-branch\n'; \
            return 64; \
        fi; \
        current_branch=$(git current-branch); \
    ...
    unpublish-branch = "!f() { \
        if [ $# -ne 0 ]; then \
            printf >&2 'Usage: git unpublish-branch\n'; \
            return 64; \
        fi; \
        current_branch=$(git current-branch); \
...
#+begin_src conf
    ...
    ahead-of = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git ahead-of BRANCH\n'; \
            return 64; \
        fi; \
        BRANCH=\"$1\"; \
...
#+begin_src conf
    ...
    behind = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git behind BRANCH\n'; \
            return 64; \
        fi; \
        BRANCH=\"$1\"; \
...
#+begin_src conf
    ...
    changed-files-since = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git changed-files-since BRANCH\n'; \
            return 64; \
        fi; \
        BRANCH=\"$1\"; \
...
#+begin_src conf
    ...
    missing = "!f() { \
        if [ $# -gt 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git missing [BRANCH]\n'; \
            return 64; \
        fi; \
        if [ $# -eq 1 ]; then \
            BRANCH=\"$1\"; \
...
#+begin_src conf
    ...
    merge-check = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git merge-check BRANCH_TO_MERGE\n'; \
            return 64; \
        fi; \
        BRANCH_TO_MERGE=\"$1\"; \
...
#+begin_src conf
    ...
    merge-tree-conflicts = "!f() { \
        if [ $# -ne 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git merge-tree-conflicts BASE_BRANCH FEATURE_BRANCH\n'; \
            return 64; \
        fi; \
        BASE_BRANCH=\"$1\"; \
        FEATURE_BRANCH=\"$2\"; \
