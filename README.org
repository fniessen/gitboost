...
#+begin_src conf
    ...
    ##?
    ##? Compare a commit with its immediate parent using the configured difftool.
    ##?
    ##? Examples:
...
#+begin_src conf
    ##? Usage: git ediff-commit COMMIT FILE_PATH
    ##?
    ##? Compare FILE_PATH between the current version and COMMIT using
    ##? Emacs' ediff.
    ediff-commit = "!f() { \
        COMMIT=\"$1\"; \
        FILE_PATH=\"$2\"; \
        current_file=\"$FILE_PATH\"; \
        git show ${COMMIT}:\"$FILE_PATH\" > /tmp/ediff-temp-${COMMIT}; \
        emacs --eval \"(ediff-files \\\"$current_file\\\" \\\"/tmp/ediff-temp-${COMMIT}\\\")\"; \
        rm /tmp/ediff-temp-${COMMIT}; \
    }; f"
#+end_src
...
: git difftool-last-modified Path/to/File.cpp
