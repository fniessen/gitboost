...
branches, use the following alias:
...
#+begin_warning
Before using the following alias, please ensure you *update all local
branches*. This requires checking out each branch in succession.
#+end_warning

*************** TODO Or use another alias for updating all local branches...

Use the following alias to show the *most recent commits* for a specific file or
directory across all local branches:

#+begin_src conf
    ##? Usage: git file-last-commit-all-local-branches FILE_PATH
    ##?
    ##? For each local branch, show only the most recent commit that touched
    ##? this file, sorted by date.
    file-last-commit-all-local-branches = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git file-last-commit-all-local-branches FILE_PATH\n'; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
        ...
        for branch in $local_branches; do \
            printf \"$(git log --follow --decorate=no -1 \
                               --color=always $branch -- \"$FILE_PATH\") $(tput bold)$(tput setaf 2)$branch$(tput sgr0)\n\"; \
        done | sort -r | sed 's#^[0-9]*:##g'; \
    }; f"

    ##? Alias for 'file-last-commit-all-local-branches'.
    wwflu = file-last-commit-all-local-branches
    # (Where Was File Last Updated?)
#+end_src

: git file-last-commit-all-local-branches schema/ab-products/bldgops/common/console/ab-bldgops-console-wr-ds.axvw

#+begin_src conf
    file-last-commit-all-local-branches2 = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf >&2 'Usage: git file-last-commit-all-local-branches2 FILE_PATH\n'; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
