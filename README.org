#+TITLE:     Mastering Git: Awesome aliases and time-saving commands
#+AUTHOR:    Fabrice Niessen
#+EMAIL:     (concat "fniessen" at-sign "pirilampo.org")
#+DESCRIPTION: A comprehensive guide to GitBoost -- a curated collection of optimized Git aliases and workflows for daily development, from basic status checks to advanced automation.
#+KEYWORDS:  git, aliases, productivity, workflow, tutorial, configuration, automation, devops
...
#+PROPERTY:  header-args :tangle .gitconfig_gitboost :eval no
...
Welcome to GitBoost!  This repository contains a collection of configuration
settings and aliases to enhance your Git workflow. Whether you're a beginner or
a seasoned Git user, you'll find time-saving shortcuts and customization options
to *boost your productivity*. Browse the contents, try them out, and feel free to
contribute your own enhancements.

#+begin_quote
[!TIP]

*** Level up your Git skills in Paris on 26 November 2025!
Gain full control of your workflow with the exclusive "GitBoost" course.

Visit https://gitboost.fr/ for details and registration,
and to /secure your place today/.
#+end_quote

* Shell settings

#+begin_src shell :tangle bin/rc-common-settings-gitboost
...
If Git is installed on your system, an alias is set up so you can type ~g~ instead
of ~git~ on the command line.

#+begin_src shell :tangle bin/rc-common-settings-gitboost
# Check if 'git' command is available.
if ! command -v git >/dev/null 2>&1; then
    printf "ERROR: 'git' is not installed. Please install it.\n" >&2
...
If the ~zip~ command isn't found, you'll see an error message prompting you to
install it.

#+begin_src shell :tangle bin/rc-common-settings-gitboost
# Check if 'zip' command is available.
if ! command -v zip >/dev/null 2>&1; then
    printf "\033[1;33mWarning: 'zip' is not installed. Please install it.\033[0m\n"
fi
#+end_src

** cdroot alias

This alias lets you switch to the root directory of your Git repository. It uses
~git rev-parse --show-toplevel~ to find the repository's root and won't throw an
error if you're not in a Git repository.
...
** Filtering ~.git~ from Grep output
...
# -*- mode: conf -*-
# .gitconfig_gitboost, Time-stamp: <<current-time()>>
#+end_src
...
When a merge results in conflicts, ~git mergetool~ can launch a graphical merge
tool to help resolve them.
...
- ~$LOCAL~ is the version from the *current branch* (the branch you have checked
  out).

- ~$REMOTE~ is the version from the *branch being merged*.

- ~$BASE~ is the *common ancestor* of ~$LOCAL~ and ~$REMOTE~.

- ~$MERGED~ is the working file that Git expects you to edit and save as the
  conflict resolution result.

*** Recommended merge tools

Choosing the right tool:

- Small changes: ~git diff~, ~vc-diff~
- Medium changes: Magit
- Large or complex merges: *Ediff*, *Meld*, *VS Code* Merge Editor, or *DeltaWalker*

*** Use Emacs (Ediff) as a Git merge tool
...
[mergetool "ediff"]
    # Launch Ediff, starting Emacs if no server is running.
    cmd = emacsclient -a emacs --eval "(ediff-merge-files-with-ancestor \\\"$LOCAL\\\" \\\"$REMOTE\\\" \\\"$BASE\\\" nil \\\"$MERGED\\\")"

    trustExitCode = true

    # Remove backup files such as *.orig after a successful merge.
    keepBackup = false
#+end_src

The ~-a ""~ option starts Emacs automatically if no Emacs server is running.

*** Use Meld as Git merge tool
...
The most common configuration uses ~$MERGED~ as the center pane and as the
editable merge result. This lets you *start from Git's partial merge result* and
*save the final conflict resolution* back to the file Git expects.

For complex merges, you may optionally add supplementary diff views showing how
~$LOCAL~ and ~$REMOTE~ evolved from ~$BASE~:
...
*** Use VS Code as Git merge tool

VS Code uses its built-in Merge Editor when resolving conflicts.
...
Select a diff tool to visualize differences outside the terminal.

#+begin_src conf
[difftool]
    # Set prompt to false to avoid asking for confirmation before launching the
    # diff tool.
    prompt = false
#+end_src

Git supports many external diff tools, including terminal-based tools such as
~vimdiff~ and graphical applications such as Meld, Sublime Merge, and Visual
Studio Code.  Choose one according to your preferences and the tools available
on your system.

*** Use Emacs as Git diff tool
...
The ~git-ediff~ helper is a custom wrapper script around ~ediff-files~. If you do
not use such a helper, use one of the alternative commands shown below.
...
[difftool "meld"]
    # Launch Meld with the two files to compare.
...
*** Use Sublime Merge as Git diff tool

Open the diff directly in Sublime Merge:
...
[difftool "smerge"]
    # Adjust this command depending on the Sublime Merge version and platform.
    cmd = smerge "$LOCAL" "$REMOTE"
#+end_src

If the ~smerge~ command is not available in your shell's search path, replace it
with the full path to the Sublime Merge executable.

#+begin_src conf :tangle no
    # cmd = "C:/Program Files/Sublime Merge/smerge.exe" "$LOCAL" "$REMOTE"
    # cmd = '/c/Program Files/Sublime Merge/smerge.exe' "$LOCAL" "$REMOTE"
#+end_src
...
[core]
    ...
    # Reuse a running Emacs server, or start Emacs if none is available.
    editor = emacsclient -a emacs
#+end_src

To *test the ~editor~ setting*, run a Git command that launches an editor, such as
~git commit --amend~, ~git revert~, ~git squash~, or ~git record~.
...
*** Show inline word differences

Use ~--word-diff=plain~ to mark additions and deletions with explicit delimiters
(~{+...+}~ and ~[-...-]~).
...
git diff --no-index \
    --word-diff=plain \
    --word-diff-regex='[^,]+' \
    <(head -n 6 "$CSV1" | sed 's_,_, _g') \
    <(head -n 6 "$CSV2" | sed 's_,_, _g')
#+end_src

*** Compare files with 'wdiff'

~wdiff~ is another tool for displaying word-level differences. Here, ANSI escape
sequences are used to color deletions in red and additions in green.
...
- *Checkout Windows-style, commit Unix-style line endings*
  ...
  #+begin_warning
  It's important to be cautious when using it in a repository with *shell scripts*
  to prevent unintentional line ending conversions that could break the shell
  scripts (designed to run in Linux and macOS systems) and lead to syntax
  errors.

  To avoid issues with shell scripts, it's often recommended to set ~autocrlf~ to
  ~input~. This setting will ensure that Git doesn't modify line endings in the
  repository but still normalizes them when checking out files on your local
  machine. This way, thei line endings of your shell scripts won't be altered by
  Git.
  #+end_warning
...
List all files whose end-of-line differs between index and working tree:

#+begin_src conf
[alias]
    eol-diff = "!git ls-files --eol | awk -F'[ /]+' '$2 != $4 {print $NF}'"
#+end_src
...
Choose the default behavior of ~git pull~ when you have divergent branches and
need to reconcile them:

- *Merge*

  *Fast-forward* the current branch to the fetched branch *if possible*, *otherwise*
...
- *Rebase*
  ...
  If there are no local commits to rebase, Git will perform a fast-forward
  merge, effectively moving your branch pointer to the upstream branch.

  Rebase with auto-abort (if it encounters conflicts):
...
- Mimics CVS-style ~update~ behavior.
- Merge conflicts are resolved similarly, but you finish with
...
This format is generally preferable because it maintains the prominence of the
*commit message* while still clearly indicating the tag and author
information. The commit message is the most important piece of information and
...
#+begin_src conf
    ...
    grep-aliases = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git grep-aliases PATTERN\n' >&2; \
            return 64; \
        fi; \
        PATTERN=\"$1\"; \
...
#+begin_src conf :tangle no
    ##? Usage: git h ALIAS_NAME
    ##?
    ##? Display the help for a custom Git alias.
    ##?
    ##? Arguments:
    ##?   ALIAS_NAME                    The name of the custom Git alias.
    h = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git h ALIAS_NAME\n' >&2; \
            return 64; \
        fi; \
        ALIAS_NAME=\"$1\"; \
        alias_description=\"$(git config alias.$ALIAS_NAME)\"; \
        if [ -z \"$alias_description\" ]; then \
            printf 'ERROR: Alias not found\n' >&2; \
            return 2; \
        fi; \
        alias_help=\"$(git config alias.$ALIAS_NAME | sed -n '/^##?/,/^$/p')\"; \
        if [ -z \"$alias_help\" ]; then \
            printf 'ERROR: No help found for the alias\n' >&2; \
            return 2; \
        fi; \
        printf \"$alias_help\"; \
    }; f"
    ...
    # Usage: git h ALIAS_NAME
    # Display the help comments for a custom Git alias.
    #
    # Arguments:
    #   ALIAS_NAME    The name of the custom Git alias.
    h = "!f() { \
        ...
        ALIAS_NAME=\"$1\"; \
        gitconfig_path=\"$(git root)/.git/config\"; \
        alias_comments=\"$(awk -v alias=\"$ALIAS_NAME\" \
            '/\\[alias\\]/ { in_alias = 1; next } \
            in_alias && /^\\s*$/ { in_alias = 0; next } \
            in_alias && $1 == alias { \
                while (getline) { \
                    if ($0 ~ /^##? /) { \
                        sub(/^##? /, \"\", $0); \
        ...
        if [ -z \"$alias_comments\" ]; then \
            printf 'ERROR: No comments found for the alias\n' >&2; \
            return 2; \
        fi; \
        printf \"$alias_comments\"; \
    }; f"
...
*Sh* scripting can be a powerful tool for automating tasks, but it's important to
follow best practices to ensure that your scripts are efficient, reliable, and
maintainable. Here are some best practices to keep in mind:

- Use variables to store values used multiple times.

- Use lowercase naming convention for local variables.
...
- Use the *single square bracket* ~[~ for conditional testing, which is more
  universally supported.

  #+begin_note
  ~[[~ has fewer surprises and is generally safer to use. But it is not
  portable -- it's a *Bashism*, POSIX doesn't specify what it does.
...
#+begin_src conf
    ...
    init-repository = "!f() { \
        git init || { \
            printf 'ERROR: git init failed. Aborting.\n' >&2; \
            return 2; \
        }; \
...
#+begin_src conf
    ##? Usage: git origin
    ##?
    ##? Print the remote name of the current branch's upstream.
    ##? Produces no output if the current branch has no upstream.
    origin = "!f() { \
...
The command intentionally *propagates failures*, causing the alias to return
immediately with a non-zero status (return code propagation).
#+end_tip

Updating Git aliases for different *main branch* names (~main~, ~master~, etc.):
...
Consider using
: git symbolic-ref --short refs/remotes/$(git origin)/HEAD 2>/dev/null | sed 's_^refs/heads/__'
?
...
#+begin_src conf :tangle no
    # Usage: git create-repository-gh REPO_OWNER REPO_SLUG_REMOTE
    create-repository-gh = "!f() { \
        REPO_OWNER=\"$1\"; \
        REPO_SLUG_REMOTE=\"$2\"; \
        curl -u \"$REPO_OWNER\" https://api.github.com/user/repos -d \"{\\\"name\\\":\\\"$REPO_SLUG_REMOTE\\\"}\"; \
    }; f"
#+end_src
...
Ubuntu's default repositories may not provide the most recent version of Git. To
get the latest version, follow these steps:

#+begin_src shell :tangle no
# Check the current version of Git.
git --version

# Add the Git-Core Personal Package Archive (PPA).
...
# Install the latest version of Git (from the PPA).
...
: printf "# %s\n\nDescription\n" "${$(git repo-slug-remote):-Repository name}" >> README.md
: git add README.md
...
#+begin_src conf
    ...
    search-for-files = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git search-for-files PATTERN\n' >&2; \
            return 64; \
        fi; \
        PATTERN=\"$1\"; \
...
XXX Put directory names in blue color.
...
#+begin_src conf
    ...
    search-for-files-all-branches = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git search-for-files-all-branches PATTERN\n' >&2; \
            return 64; \
        fi; \
        PATTERN=\"$1\"; \
        git remote-branches \
...
#+begin_src conf
    find-files-all-branches2 = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git find-files-all-branches2 PATTERN\n' >&2; \
            return 64; \
        fi; \
...
#+begin_src conf
    ##? Usage: git grep-current-branch PATTERN [-- PATHSPEC]
    ##?
    ##? Perform a case-insensitive search for PATTERN in all tracked files of
    ##? the currently checked-out branch. Shows line numbers, file headers, and
    ##? one line of context around each match.
    ##?
    ##? Examples:
    ##?   git grep-current-branch SELECT
    ##?   git grep-current-branch SELECT -- '*.sql'
    ##?                                 Search for "SELECT" only in .sql files.
    ##?   git grep-current-branch error -- src/
    grep-current-branch = "!f() { \
        test $# -ge 1 || { printf 'Usage: git grep-current-branch PATTERN [-- PATHSPEC]\\n' >&2; exit 2; }; \
        PATTERN=\"$1\"; shift; \
        git --no-pager grep -i -I -n --break --heading -C 1 -e \"$PATTERN\" \"$@\"; \
      }; f"

    # Aliases for 'grep-current-branch'.
    grep-all-tracked-files = grep-current-branch
    search-for-code = grep-current-branch
    ack = grep-current-branch

    grep-current-branch-case = "!f() { \
        test $# -ge 1 || { printf 'Usage: git grep-current-branch-case PATTERN [-- PATHSPEC]\\n' >&2; exit 2; }; \
        PATTERN=\"$1\"; shift; \
        git --no-pager grep -I -n --break --heading -C 1 -e \"$PATTERN\" \"$@\"; \
    }; f"
#+end_src
...
#+begin_src conf
    # Usage: git grep-local-branches-in-extension EXTENSION PATTERN
    #
    # Search for PATTERN in files with the given extension across all local
    # branches.
    #
    # Arguments:
    #   EXTENSION                      The file extension to search within (without dot).
    #   PATTERN                        The code pattern to search for.
    grep-local-branches-in-extension = "!f() { \
        if [ $# -ne 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git grep-local-branches-in-extension EXTENSION PATTERN\n' >&2; \
            return 64; \
        fi; \
        EXTENSION=\"$1\"; \
        PATTERN=\"$2\"; \
...
#+begin_src conf
    ##? Usage: git grep-all-local-branches PATTERN [-- PATHSPEC]
    ##?
    ##? Search PATTERN across all local branch tips (no full history).
    ##? Faster, with far fewer duplicates than grep-all-commits.
    grep-all-local-branches = "!f() { \
      test $# -ge 1 || { printf 'Usage: git grep-all-local-branches PATTERN [-- PATHSPEC]\\n' >&2; exit 2; }; \
      PATTERN=\"$1\"; shift; \
      git for-each-ref --format='%(refname:short)' refs/heads \
        | xargs -r -n 64 sh -c 'git --no-pager grep -n --break --heading -I -e \"$0\" \"$@\"' \"$PATTERN\" \"$@\"; \
    }; f"

    ##? Usage: git grep-all-commits PATTERN [-- PATHSPEC]
    ##?
    ##? Search PATTERN through the entire commit history (all commits).
    ##? Note: results can appear many times across different commits.
    grep-all-commits = "!f() { \
      test $# -ge 1 || { printf 'Usage: git grep-all-commits PATTERN [-- PATHSPEC]\\n' >&2; exit 2; }; \
      PATTERN=\"$1\"; shift; \
      git rev-list --all \
        | xargs -r -n 256 sh -c 'git --no-pager grep -n --break --heading -I -e \"$0\" \"$@\"' \"$PATTERN\" \"$@\"; \
    }; f"
#+end_src
...
#+begin_src conf
    ...
    # Search for a pattern in all tracked files, ignoring case and binary files.
...
*WARNING Unmodify ne marche qu'avec un FILE_PATH complet, depuis la racine du repo*
...
#+begin_src conf
    ...
    unmodify = "!f() { \
        if [ $# -gt 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git unmodify [FILE_TO_RESTORE]\n' >&2; \
            return 64; \
        fi; \
        FILE_TO_RESTORE=\"$1\"; \
        if [ -z \"$FILE_TO_RESTORE\" ]; then \
            ...
            if git status --porcelain \"$FILE_TO_RESTORE\" | grep -qE '^ M'; then \
                ...
                printf \"ERROR: File '%s' is not modified.\n\" \"$FILE_TO_RESTORE\" >&2; \
                return 2; \
            fi; \
        fi; \
    }; f"
    ...
    undelete = "!f() { \
        if [ $# -gt 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git undelete [FILE_TO_RESTORE]\n' >&2; \
            return 64; \
        fi; \
        FILE_TO_RESTORE=\"$1\"; \
        if [ -z \"$FILE_TO_RESTORE\" ]; then \
...
#+begin_note
The two dashes (~--~, often referred to as the "bare double dashes" or the "end of
options" character) serve the purpose of signaling to the ~checkout~ command that
we intend to roll back a file and not switch branches, which is the conventional
use of the ~checkout~ command.
#+end_note
...
How to recover deleted files in the Git repository?

: git deleted-files -z | xargs -0 git checkout --

How to remove deleted files from the Git repository?

: git deleted-files -z | xargs -0 git rm

List *all* of the *files* in the working directory that are *both ignored and
untracked*. It's useful for quickly identifying all files that Git does not
currently manage.
...
List all of the files in the working directory that are not tracked by Git (and
are not listed in the =.gitignore= file).
...
List only *ignored files* (according to your Git repository's =.gitignore= rules and
patterns):
...
This appends all untracked files (except =.gitignore= itself) to your =.gitignore=.
...
#+begin_warning
Be careful when using the ~git clean~ command, as it cannot be undone.
#+end_warning

| Alias                | Removes untracked | Removes ignored | Danger level |
|----------------------+-------------------+-----------------+--------------|
| clean-only-untracked | Yes               | No              | Low          |
| clean-only-ignored   | No                | Yes             | Medium       |
| clean-all            | Yes               | Yes             | High         |

*** Remove only untracked files and directories

Delete all untracked files, like ~hg purge --unknown~.

#+begin_src conf
    ##? Usage: git clean-only-untracked
    ##?
    ##? Remove only untracked files and directories (confirmation required).
    ##? THIS COMMAND MAY DELETE FILES THAT CANNOT BE RECOVERED.
    clean-only-untracked = "!f() { \
      printf '\\033[1;33mWARNING: This will permanently delete all untracked files and directories.\\033[0m\\n'; \
      printf 'Tracked files and ignored files will remain untouched.\\n'; \
...
- This will *delete all untracked files and directories*.
- *Warning:* This operation cannot be undone!

#+begin_src conf
    ##? Usage: git clean-only-untracked-dry-run
    ##?
    ##? Show what would be deleted by git clean-only-untracked.
    clean-only-untracked-dry-run = clean --dry-run -f -d
#+end_src

*** Delete only ignored files and directories

To remove only ignored files:

: git ignored-files -z | xargs -0 rm

To *remove only ignored files and directories*, use the following command:

: git clean-only-ignored

This command removes all files and directories specified in =.gitignore=, while
keeping *untracked files untouched*. Any staged changes, including modified but
uncommitted files, remain unaffected.

It is commonly used to clean up ignored build artifacts or temporary files.

#+begin_src conf
    ##? Usage: git clean-only-ignored
    ##?
    ##? Remove only ignored files and directories (confirmation required).
    ##? THIS COMMAND MAY DELETE FILES THAT CANNOT BE RECOVERED.
    clean-only-ignored = "!f() { \
      printf '\\033[1;33mWARNING: This will permanently delete all ignored files and directories.\\033[0m\\n'; \
      printf 'Are you sure? [y/N] '; \
      read -r confirm; \
      if [ \"$confirm\" = y ] || [ \"$confirm\" = Y ]; then \
        git clean -f -d -X; \
...
#+begin_warning
These commands are irreversible and can lead to permanent data loss if used
carelessly. Proceed with caution.
#+end_warning

**** Check before deleting ignored files

To preview what files and directories would be removed by ~git clean-only-ignored~
without actually deleting them, use:

: git clean-only-ignored-dry-run

#+begin_src conf
    ##? Usage: git clean-only-ignored-dry-run
    ##?
    ##? Show what would be deleted by git clean-only-ignored.
    clean-only-ignored-dry-run = clean --dry-run -d -X
#+end_src

*** Delete all ignored and untracked files and directories

For a more aggressive cleanup that *removes both ignored and untracked files and
directories*, use:

: git clean-all

This command is especially dangerous because it removes *all* files and
directories that Git does not know about (including ignored files,
work-in-progress files or configuration files not committed to the
repository). Be cautious as *deleted files cannot be recovered* unless you have
a backup.

#+begin_src conf
    ##? Usage: git clean-all
    ##?
    ##? Remove all ignored and untracked files and directories (confirmation
    ##? required).
    ##? THIS COMMAND MAY DELETE FILES THAT CANNOT BE RECOVERED.
    clean-all = "!f() { \
      printf '\\033[1;33mWARNING: This will permanently delete *all* ignored and untracked files and directories!\\033[0m\\n'; \
...
#+begin_note
The behavior of ~git clean~ differs from ~git reset --hard~, which does not affect
untracked files.
#+end_note

**** Check before deleting ignored and untracked files

To preview what files and directories would be removed by ~git
clean-all~ without actually deleting them, use:

: git clean-all-dry-run

#+begin_src conf
    ##? Usage: git clean-all-dry-run
    ##?
    ##? Preview untracked and ignored files that a full clean (git clean-all)
    ##? would remove.
    clean-all-dry-run = clean --dry-run -d -x

    # Alias for 'clean-all-dry-run'.
    clean-all-preview = clean-all-dry-run
#+end_src

*** Safeguarding your work before cleaning

Before running any of the above commands, it's a good idea to save the current
state of your branch. Follow these steps:

#+begin_src conf :tangle no
# Create a new branch named "my-work-backup" and switch to it.
git checkout -b my-work-backup

# Stage all changes (including new, modified, and deleted files).
git add -A

# Commit the staged changes with a message describing the commit.
git commit -m "Safe commit: Preserve current state"

# Switch back to the original branch.
git checkout ORIGINAL_BRANCH_NAME
#+end_src

*Note:* Always use the ~-dry-run~ variants of the commands first to verify which
files will be deleted.

*** Hg shelve untracked files

*Git does not natively stash untracked files by default, but you can use:*
...
- This will stash both modified and untracked files.
- If you want to stash ONLY untracked files, see the note below.

**** Notes

If you want to *stash only untracked files* (and not modified tracked files), you
can use:
...
Include issue keys in your commit messages to link them to your issues.

: git commit -m "UBSW-985 <message>"
...
** Interactive committing (Cherry-pick style recording)

#+begin_src conf
[alias]
#+end_src

For more granular control over committing changes interactively, you can use the
~commit-interactive~ alias. This allows you to interactively stage and *commit
patch hunks* within specific files.
...
#+begin_src conf
    ...
    commit-interactive = "!f() { \
        if [ \"$(git root)\" != \"$(pwd)\" ]; then \
            printf 'ERROR: Not at the root of the Git repository.\n' >&2; \
            return 2; \
        fi; \
...
To use it, navigate to your repository root and invoke git record followed by
the file(s) you wish to interactively stage and commit. If you decide to abandon
the commit during staging (~git add -p~), the alias automatically resets the index
...
This alias retrieves the full hash of the current commit (~HEAD~) in the working
directory.
...
*************** TODO More information with lastn2 (for file renames), but missing newline after merge commits...

#+begin_src conf :tangle no
    lastn3 = "!f() {
        git show -m --name-status --stat -M $  (git rev-list -  ${1:-1} HEAD); 
    }; f"

    # Montre les fichiers modifiés par le dernier commit (ou N derniers)
    # Pour les merges → affiche uniquement ce qu'a apporté la branche mergée
    lastn3 = "!f() { \
        n=${1:-1}; \
        for c in $(git rev-list -n $n HEAD); do \
            if git rev-parse --verify \"$c^2\" >/dev/null 2>&1; then \
                echo \"$(tput bold)$(tput setaf 3)Merge commit $c$(tput sgr0) -- changes from merged branch:\"; \
                git show --name-status --stat -M \"$c^2\"..\"$c\"; \
            else \
                echo \"$(tput bold)Commit $c$(tput sgr0):\"; \
                git show --name-status --stat -M \"$c\"; \
            fi; \
            echo; \
        done; \
    }; f"
...
#+begin_src conf
    ...
    file-history = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git file-history FILE_PATH\n' >&2; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
        if ! git ls-tree -r HEAD --name-only 2>/dev/null | grep -q \"^$FILE_PATH$\"; then \
            printf \"\\e[1;33mWarning: File '%s' not found in the current branch of the repository.\\e[0m\n\" \"$FILE_PATH\"; \
        fi; \
...
: curl -u USER_NAME 'https://api.bitbucket.org/2.0/repositories/REPO_OWNER/REPO_SLUG_REMOTE/filehistory/master/PATH/TO/FILE'
...
#+begin_src conf
    ...
    ##?
    ##? Show the complete history of this file (or directory) across the whole repo.
    file-history-all-branches = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git file-history-all-branches FILE_PATH\n' >&2; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
        if ! git ls-tree -r HEAD --name-only 2>/dev/null | grep -q \"^$FILE_PATH$\"; then \
...
The output of
: git file-history-all-branches db/aremis/Sprint_2025_13/UBSW-1549_02_int_sync_04_employees.sql
is not sorted!?

# Tests:
# : git file-history-all-branches db/aremis/Sprint_2024_07/GCS-4307_2_BBS_ROLES_roles.sql
# : git file-history-all-branches db/aremis/Sprint_2024_07/GCS-4307_BBS_ROLES_FOR_S2_STACK-2.sql

*** Related files (commit history and associated file changes)
...
#+begin_src conf
    ...
    related-history = "!f() { \
        FILE_PATH=\"$1\"; \
        if [ ! -e \"$FILE_PATH\" ]; then \
            printf \"ERROR: File '%s' not found in the working tree.\n\" \"$FILE_PATH\" >&2; \
            return 2; \
        fi; \
...
# ~\n~ should be escaped. For instance, the given command:
#
# #+begin_src conf
#     related-history = "!f() { \
#         git log --pretty=format:\"%h %ad %s <%an>\" --date=short --name-status -- \"$1\" \
#         | awk ' \
#             BEGIN { RS = \"\"; FS = \"\n\"; OFS = \"\\n\" } \
#             { print $1, $2; for (i=3; i<=NF; i++) { if ($i ~ /^R/) { split($i, arr, \"\\t\"); print \"Renamed: \" arr[3]; } else if ($i ~ /^D/) { split($i, arr, \"\\t\"); print \"Deleted: \" arr[2]; } else { print $i; } } print \"\" } \
#         '; \
#     }; f"
# #+end_src
#
...
#
# : awk: cmd. line:1:          BEGIN { RS = ""; FS = "
# : awk: cmd. line:1:                                ^ unterminated string
# : awk: cmd. line:1:          BEGIN { RS = ""; FS = "
# : awk: cmd. line:1:                                ^ syntax error
#
# To avoid this error, escape the backslash and the newline character properly.
...
#+begin_src conf
    ...
    related-files = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git related-files FILE_PATH\n' >&2; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
        if [ ! -e \"$FILE_PATH\" ]; then \
...
To find the most recent commit (chronologically) that modified a file across /all/
known branches, use the following alias:
...
#+begin_src conf
    ...
    file-last-commit = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git file-last-commit FILE_PATH\n' >&2; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
...
The ~--all~ option searches all references known to the local repository,
including:

- local branches,
- remote-tracking branches (for example ~origin/main~),
- tags.

To ensure the results reflect the latest state of the remote repositories,
*update your references* first:

#+begin_src shell :tangle no
git fetch --all --prune
#+end_src

Use the following alias to show the *most recent commits* for a specific file or
directory across all known branches:

#+begin_src conf
    ##? Usage: git file-last-commit-all-branches FILE_PATH
    ##?
    ##? For each local branch, show only the most recent commit that touched
    ##? this file, sorted by date.
    file-last-commit-all-branches = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git file-last-commit-all-branches FILE_PATH\n' >&2; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
        ...
        for ref in $refs; do \
            ...
            printf \"$(git log --follow --decorate=no -1 \
                               --color=always \"$ref\" -- \"$FILE_PATH\") $(tput bold)$(tput setaf 2)$ref$(tput sgr0)\n\"; \
        done | sort -r | sed 's_^[0-9]*:__g'; \
    }; f"

    ##? Alias for 'file-last-commit-all-branches'.
    wwflu = file-last-commit-all-branches
    # (Where Was File Last Updated?)
#+end_src

: git file-last-commit-all-branches schema/ab-products/bldgops/common/console/ab-bldgops-console-wr-ds.axvw

#+begin_src conf
    file-last-commit-all-local-branches-2 = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git file-last-commit-all-local-branches-2 FILE_PATH\n' >&2; \
            return 64; \
        fi; \
        FILE_PATH=\"$1\"; \
...
** How to use time-based Git filters with Zsh aliases

These global aliases (for Zsh) use ~G~ (for Git) as the prefix, followed by ~D~ for
day, ~W~ for week, ~M~ for month, and ~Y~ for year.

#+begin_src shell :tangle bin/rc-zsh-specific-settings-gitboost
# Hey Emacs, this is a -*- sh -*- file ...
# NOTE: This file is meant to be sourced (e.g. from .bashrc or .zshrc)
...
alias -g G1D=' --since=1.day.ago'
alias -g G1W=' --since=1.week.ago'
alias -g G1M=' --since=1.month.ago'
alias -g G3M=' --since=3.months.ago'
alias -g G1Y=' --since=1.year.ago'

# Global alias to exclude .git directories/files from grep results.
alias -g NG="| grep -v '\.git/'"        # "No Git".
#+end_src

You can use these aliases in Git commands to filter commits based on time, for
example:

: git log GW

This will show Git logs for commits made within the last week.
...
#+begin_src conf :tangle no
    changed-files-m = "!f() { \
        COMMIT=${1:-HEAD}; \
        git show -m --name-status --stat -M \"$COMMIT\"; \
    }; f"
#+end_src

** Get the diff of a commit

Display the changes introduced by a commit with word-level or character-level
highlighting.
...
: git show 23231bf66
=
: git difftool 23231bf66^..23231bf66
...
#+begin_src conf
    ##? Usage: git difftool-parent [COMMIT]
    ##?
    ##? Compare a commit (default HEAD) with its immediate parent using the
    ##? configured difftool.
    ##?
    ##? Examples:
    ##?   git difftool-parent            # Compare HEAD^..HEAD
    ##?   git difftool-parent a1b2c3d    # Compare that commit with its parent
    ##?
    ##? Notes:
    ##? - Falls back to HEAD if no commit hash is provided.
    ##? - Supports any extra difftool options after the commit hash.
    difftool-parent = "!f() { \
        COMMIT=${1:-HEAD}; \
        git difftool $COMMIT^..$COMMIT; \
    }; f"

    # Alias for 'difftool-parent'.
    dftp = difftool-parent
#+end_src
...
: git difftool COMMIT^..COMMIT -- PATH/TO/FILE

#+begin_src conf
    ##? Usage: git difftool-file-parent COMMIT FILE_PATH
    ##?
    ##? Compare a specific file between a given commit and its immediate parent,
    ##? using the configured difftool.
    ##?
    ##? This shows how FILE_PATH changed in COMMIT.
    difftool-file-parent = "!f() { \
        [ $# -eq 2 ] || { printf 'Usage: git difftool-file-parent COMMIT FILE_PATH\\n' >&2; exit 64; }; \
        COMMIT=\"$1\"; \
        FILE_PATH=\"$2\"; \
        git difftool \"COMMIT^..COMMIT\" -- \"FILE_PATH\"; \
    }; f"

    # Alias for 'difftool-file-parent'.
    dftfp = difftool-file-parent
#+end_src

Diff the last commit for a given file:

#+begin_src conf
    ##? Usage: git difftool-file-last FILE_PATH
    ##?
    ##? Open the difftool for the most recent change of FILE.
    ##?
    ##? Notes:
    ##? - Finds the last commit that modified FILE_PATH using 'git log -1'.
    ##? - Then runs 'git difftool COMMIT^..COMMIT -- FILE_PATH' to show the change.
    difftool-file-last = "!f() { \
        [ $# -ge 1 ] || { printf 'Usage: git difftool-file-last FILE_PATH [-- <options>]\\n' >&2; exit 64; }; \
        FILE_PATH=$1; \
        LAST_COMMIT=$(git log -1 --format=%H -- \"$FILE_PATH\") || exit 1; \
        if [ -z \"$LAST_COMMIT\" ]; then \
            printf 'Error: No commits found for %s\\n' \"$FILE_PATH\" >&2; \
            exit 1; \
        fi; \
        printf 'Last commit modifying %s:\\n' \"$FILE_PATH\"; \
        git log -1 --format='  %C(yellow)%h%Creset %Cgreen%an%Creset %Cblue%ar%Creset %s' -- \"$FILE_PATH\"; \
        git difftool \"$LAST_COMMIT^..$LAST_COMMIT\" -- \"$FILE_PATH\"; \
    }; f"

    # Alias for 'difftool-file-last'.
    dftfl = difftool-file-last
#+end_src

Usage:

: git difftool-file-last path/to/File.cpp

This opens your configured ~difftool~ with the changes to =path/to/File.cpp= in the
last commit it was touched.
...
#+begin_src conf
    ...
    diff-commits-ago = "!f() { \
        if [ $# -lt 1 ] || [ $# -gt 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git diff-commits-ago NUMBER [FILE]\n' >&2; \
            return 64; \
        fi; \
        if [ $# -eq 2 ]; then \
...
#+begin_src conf
    ...
    # Minimal (char-level), whitespace-insensitive diff of staged changes (index vs HEAD).
    mindfc = diff -w --word-diff-regex=. --color-words -U0 --cached
#+end_src

*** Graphical diff tool

For users who prefer a graphical comparison tool instead of terminal output:

#+begin_src conf
    # Graphical diff (working tree vs index).
    dt = difftool

    # Graphical diff of staged changes (index vs HEAD).
    dtc = difftool --cached
#+end_src

The graphical diff tool configured through =git difftool= depends on your Git
configuration (for example, Meld, Beyond Compare, KDiff3, WinMerge, VS Code,
etc.). It can be useful when reviewing complex changes, moved code, or files
that are difficult to inspect in a terminal diff.
...
Alternative: use [[https://diffoscope.org/][diffoscope]]

** Get the references the commit has been pushed to (Which branches and tags?)

This section provides commands to list all *branches and tags* that contain
a specified *commit hash* among their ancestors.

*** List all branches containing a specific commit

The following command lists all branches (both local and remote) that contain
the specified commit hash, sorted by the most recent commit date:

#+begin_src conf
    ##? Usage: git branches-containing COMMIT
    ##?
    ...
    branches-containing = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git branches-containing COMMIT\n' >&2; \
            return 64; \
        fi; \
        COMMIT=\"$1\"; \
        git branch --all --sort=-committerdate --contains \"$COMMIT\"; \
    }; f"

    # Alias for 'branches-containing'.
    who-has = branches-containing
#+end_src

This command includes:

- *Local branches:* Branches that exist in your local repository.
- *Remote-tracking branches:* Local representations of branches on remote
  repositories. These are branches that track remote branches directly.

By sorting the branches by the latest commit date, this command helps you easily
identify which branches have recently incorporated the specified commit.

**** Find the latest branch containing a specific commit

To locate the most recently updated branch (including remote branches) that
contains a given commit, use the following command:

: git branches-containing COMMIT | head -n 1

**** Find the oldest branch containing a specific commit

To identify the branch that was updated the earliest (including remote branches)
and contains a given commit, use the following command:

: git branches-containing COMMIT | tail -n 1

*** List all tags containing a specific commit

This command lists all tags that contain the specified commit hash:

#+begin_src conf
    ##? Usage: git tags-containing COMMIT
    ##?
    ##? List tags containing a specific commit, sorted by the most recent commit
    ##? date.
    tags-containing = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git tags-containing COMMIT\n' >&2; \
            return 64; \
        fi; \
        COMMIT=\"$1\"; \
...
#+begin_src conf
    ...
    is-ancestor = "!f() { \
        if [ $# -ne 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git is-ancestor COMMIT1 COMMIT2\n' >&2; \
            return 64; \
        fi; \
        COMMIT1=\"$1\"; \
        COMMIT2=\"$2\"; \
...
Show commits that match *any* of the following patterns
...
#+begin_src conf
    ...
    #
    # Search for commit messages that match any of the provided patterns (OR
    # logic, like in BitBucket).
    #
...
Show commits that match *all* of the following patterns

#+begin_src conf
    # Usage: git find-commit-messages-all PATTERN...
    #
    # Search for commit messages that match all provided patterns (AND logic,
    # like in Gitlab).
    #
    # This will search commit messages for the provided patterns and list the
    # matching commits.
    ...
    find-commit-messages-all = "!f() { \
        if [ $# -eq 0 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git find-commit-messages-all PATTERN...\n' >&2; \
            return 64; \
        fi; \
        CMD=\"git log --all-match -i --color=always\"; \
        for PATTERN in \"$@\"; do \
            CMD=\"$CMD --grep='$PATTERN'\"; \
        done; \
        eval $CMD | sort -r | sed 's_^[^:]*:__'; \
    }; f"
#+end_src
...
*** Search by changes in the content

There are two key differences between ~-S~ and ~-G~ in Git:

- String vs. Regular Expression:
  + ~-S~ searches for an exact string.
  + ~-G~ searches using a regular expression.

- Matching Criteria:
  + ~-S~ requires the match to appear a *different* /number of times/ on the left
    (deleted lines) and right (added lines) sides of the diff.
  + ~-G~ requires the match to appear on /at least one/ side of the diff, either in
    the added or removed lines.
...
~git log -S"frotz\(nitfol" --pickaxe-regex~ will not (because the number of
occurrences of that string did not change).

(note: the ~--pickaxe-regexp~ in that paragraph is sort of a red herring, the
example tries to match a fixed string anyway. The author of the docs could have
written: "~-G"frotz\(nitfol"~ will show this commit, ~-S"frotz(nitfol"~ will not"
/[<- the astute reader will notice the blatant absence of any '\' character in
the second pattern]/, or alternatively choose an example pattern where no
special char was involved)

**** Search for commits where a string is added or removed
...
**** Search for commits where a regexp is added, removed, or modified
...
The alias ~find-commit-diffs-all-branches~ searches for string-related changes
in the code changes (diff) across all branches.
...
: git log --all -p -- '*.sql' | grep -E -C 2 'rule_type'

** Search in file snapshots

Searches for a fixed string across the complete file tree of every commit in the
repository history. For each commit, it scans the full snapshot of all tracked
files as they existed at that point in time, not just the diffs.

The output format is: ~<short-commit-hash>:<path>:<matching line>~

It may return the same match multiple times if the file was unchanged across
several commits. Deleted files are included if they existed in past commits.

#+begin_src conf
    # Usage: git grep-string-in-all-commit-snapshots SEARCH_TERM
    grep-string-in-all-commit-snapshots = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git grep-string-in-all-commit-snapshots SEARCH_TERM\n' >&2; \
            return 64; \
        fi; \
        SEARCH_TERM=\"$1\"; \
        git rev-list --all | \
        xargs -n1 sh -c 'git grep -F \"$0\" \"$1\" | sed \"s_^$1:_$(git rev-parse --short $1):_\"' \"$SEARCH_TERM\"; \
    }; f"
#+end_src
...
#+begin_src conf
    ...
    amend-reuse = "!f() { \
        ...
        if [ -n \"$upstream_commit\" ] && [ \"$upstream_commit\" = \"$local_commit\" ]; then \
            printf 'ERROR: The commit has already been pushed to the remote repository.\n' >&2; \
            return 2; \
        fi; \
...
#+begin_src conf
    ...
    amend-message = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git amend-message NEW_COMMIT_MESSAGE\n' >&2; \
            return 64; \
        fi; \
        upstream_commit=$(git upstream-commit 2>/dev/null); \
        local_commit=$(git local-commit); \
...
#+begin_src conf
    ...
    amend-author = "!f() { \
        if [ $# -ne 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git amend-author NAME EMAIL\n' >&2; \
            return 64; \
        fi; \
        NAME=\"$1\"; \
        EMAIL=\"$2\"; \
...
#+begin_src conf
    ...
    ##? Alias for 'uncommit-unstage'.
    uncommit-unadd = uncommit-unstage
#+end_src
...
This is useful for splitting a large commit into smaller, reviewable parts.
...
#+begin_src conf
    ...
    squash = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git squash NUM_OF_COMMITS_TO_UNDO\n' >&2; \
            return 64; \
        fi; \
        ...
        if [ -z \"$upstream_branch\" ]; then \
            printf 'Warning: No upstream branch configured for the current branch.\n' >&2; \
            printf 'Consider using 'squash-force' alias.\n' >&2; \
            return 2; \
        fi; \
        ...
        if [ $NUM_OF_COMMITS_TO_UNDO -le $num_unpushed ]; then \
            ...
            printf 'ERROR: Cannot squash %d commits. There are only %d unpushed commits.\n' \
                   $NUM_OF_COMMITS_TO_UNDO $num_unpushed >&2; \
            return 2; \
        fi; \
    }; f"
#+end_src

Alias without safety checks:
...
Start an interactive rebase of all the commits you haven't pushed yet. Handy for
collapsing a bunch of work-in-progress snapshots into logical commits before
pushing, without having to worry about accidentally squashing a commit someone
...
You can use the ~git-iterate-commits~ function to loop through Git commit hashes,
which are obtained from the second column of a *previous command's output*, and
execute a specified Git command on each commit. This is particularly useful when
you need to perform operations on a series of commits.
...
git-iterate-commits() {
    ...
    eval "$previous_command" | while IFS= read -r line; do
        ...
        git "$git_command" "$commit_hash" \
            || printf "ERROR: Failed to execute 'git %s %s'\n" "$git_command" "$commit_hash" >&2
    done
}
#+end_src

For example, let's say you want to execute the ~changed-files~ Git command for
each commit reported by:

: git file-history db/aremis/mms/01_sp_mms_sync_01_staging.sql

This command will execute the specified ~changed-files~ command for each commit
hash in the series:
...
** Recent activity and standup reports

Quickly see what has been done today (list commits that aren't merges, on *all
branches*):
...
#+begin_src conf
    ...
    ##?
    ##? Summarize changes since 24h (or since last Friday on Mondays), suitable
...
Apply the changes introduced by some existing commits; useful for moving small
...
** Using git bisect to find a faulty commit

Git Bisect is an interactive tool used to identify which commit in your
project's history introduced a bug or regression. By performing a binary search
through your commit history, it efficiently narrows down the problematic commit.

*** Step-by-step guide to using git bisect

You start by specifying a range between two commits (by tag or commit hash):
- one where the issue isn't present (the "good" commit) and
- another where it is (the "bad" commit).

Then, run

: git bisect start
: git bisect bad  # HEAD (bug present)
: git bisect good abc1234

Git Bisect guides you through a binary search process by checking out the
*midpoint* between known good and bad commits. At each step, you mark the current
commit as either good (~git bisect good~) or bad (~git bisect bad~). This continues
until Git pinpoints the exact commit that introduced the bug or regression.

: 2ddcca36c8bcfa251724fe342c8327451988be0d is the first bad commit

You can review the commit to understand what changes were introduced:

After identifying the bad commit, reset your repository to the branch you were
on before starting git bisect:

: git bisect reset

*** Automate Git Bisect with a Script

You can automate the tests with a script:
...
The script ~test-script.sh~ must return:

- ~0~ if the commit is *good*
- ~1~ (or any non-zero code) if the commit is *bad*
...
#!/usr/bin/env sh
make || exit 255
./my_app | grep 'good output'
#+end_src

This approach reduces typing, improves consistency, and makes it easier to reuse
the same logic across multiple bisect sessions.

*** Using Git Bisect with Emacs and Org Files

For Emacs users working with Org-mode, you can leverage a script to test changes
during a ~git bisect~ session. This example demonstrates how to automate testing
for a specific bug ("[BUG] Marker points into wrong buffer") using an Emacs Lisp
script.

*Bash Script for git bisect run*

#+begin_src shell :tangle no
#!/usr/bin/env bash
# Run Emacs in batch mode with the test script and check its output.
emacs -l ./test.el --batch | grep 0
#+end_src

*Emacs Lisp Script (test.el)*

The Emacs Lisp script performs the following actions:
- Disables interactive prompts (~yes-or-no-p~).
- Suppresses unnecessary messages (~message~).
- Loads Org-mode's export functionality.
- Exports an Org file (=test.org=) to LaTeX to verify functionality.

#+begin_src emacs-lisp :tangle no
#!/usr/bin/emacs --script

;; Override yes-or-no prompts to always return true.
...
;; Run the test.
(progn
  (require 'ox) ; Load Org export functionality.
  ;; Load the Org file and export it to LaTeX.
  (with-temp-buffer
    (insert-file "test.org")
    (org-latex-export-as-latex))
  ;; Output a success indicator for bisect.
  (print 0))
#+end_src
...
1. Bash Script
   - Runs Emacs in batch mode with the =test.el= script.
   - Pipes the output to ~grep~ to check for the success signal (~0~).

2. Emacs Lisp Script
   - Ensures no prompts or unnecessary output interfere with automated testing.
   - Processes the Org file and exports it to LaTeX using
     ~org-latex-export-as-latex~.
   - Outputs ~0~ to indicate success, which ~git bisect~ uses to mark the commit as
     good.

*Using the scripts with git bisect*

1. Ensure the Emacs script is executable:

   : chmod +x test.el

2. Run ~git bisect~ with the Bash script:

   git bisect run ./test-script.sh

This approach combines the power of Git Bisect and Emacs scripting to
efficiently identify commits introducing bugs in workflows involving Org-mode.
...
** Multiple checkouts: isolated branch work

Advantages of using separate directories for each branch:

- Clear separation of contexts.
- Reduced risk of accidental cross-branch contamination.

/Ideal for parallel work across release lines./
...
#+begin_src conf
    ...
    create-branch = "!f() { \
        if [ $# -ne 1 ]; then \
            printf 'Usage: git create-branch BRANCH_NAME\n' >&2; \
            exit 64; \
        fi; \
        BRANCH_NAME=\"$1\"; \
...
: git revert sha1-name-of-offending-commit
#+end_note
...
Push the current branch to the remote and establish tracking:
...
#+begin_src conf
    ...
    publish-branch = "!f() { \
        if [ $# -ne 0 ]; then \
            printf 'Usage: git publish-branch\n' >&2; \
            return 64; \
        fi; \
        current_branch=$(git current-branch); \
    ...
    unpublish-branch = "!f() { \
        if [ $# -ne 0 ]; then \
            printf 'Usage: git unpublish-branch\n' >&2; \
            return 64; \
        fi; \
        current_branch=$(git current-branch); \
...
#+begin_src conf
    ...
    branches2 = "!f() { \
        ...
        git for-each-ref \
            --sort=-committerdate \
            --format='%(if)%(HEAD)%(then)*%(else) %(end) %(color:green bold)%(refname:short)%(color:reset) %(color:red)%(upstream:short)%(color:reset) %(color:red bold)%(upstream:track)%(color:reset) %(color:yellow)%(objectname:short)%(color:reset) %(contents:subject)' \
...
#+begin_src conf
    ...
    verify-local-branch = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git verify-local-branch BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
        BRANCH_NAME=\"$1\"; \
    ...
    verify-remote-branch = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git verify-remote-branch BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
        BRANCH_NAME=\"$1\"; \
...
XXX Add "no remote" for branches which are not published
...
XXX: Replace ~git branch -r --format~ with ~git for-each-ref --format~ to handle special characters in branch names.
...
#+begin_src conf
    ...
    branch-edit-desc = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git branch-edit-desc BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
        BRANCH_NAME=\"$1\"; \
    ...
    branch-show-desc = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git branch-show-desc BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
        BRANCH_NAME=\"$1\"; \
...
: git branch --edit-description --unset BRANCH_NAME
...
To manually *sync* a *branch* (merge changes from the ~main~ branch into the current
working branch), run the following command:
...
#+begin_src conf
    ...
    sync-main = "!f() { \
        ...
        if [ $merge_status -eq 0 ]; then \
            ...
            printf \"\\033[1mUnable to sync branch:\\033[0m You can't merge until you resolve all merge conflicts.\\n\" >&2; \
            return 2; \
        fi \
    }; f"
#+end_src
...
Useful for updating your local branch with the latest changes from the remote
branch while keeping your local changes intact and rebased:
...
Or, if you need more control and visibility over the stash process:
    ...
    pull-autostash-include-untracked = "!f() { \
        ...
        if [ -n \"$changes\" ]; then \
            ...
            if ! git diff --quiet && ! git diff --cached --quiet; then \
                printf \"\\033[1mStash application would result in conflicts. Resolve conflicts manually.\\033[0m\\n\" >&2; \
...
This approach ensures that the stash and pop operations are only performed if
there are changes to stash, avoiding the risk of popping an old stash.
...
is *unsafe*: If there's nothing to stash, the first command does nothing, and the
~stash pop~ command may end up applying some old, unrelated changes.
#+end_warning
...
Fetch updates, then merges the appropriate *single branch*.
...
#+begin_src conf
    ...
    has-new-commits = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git has-new-commits BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
        BRANCH_NAME=\"$1\"; \
        if [ -z \"$BRANCH_NAME\" ]; then \
            printf 'ERROR: Please provide a branch name as a parameter.\n' >&2; \
            return 64; \
        fi; \
...
Use ~delete-merged-branches~ if you need to *delete local branches merged into any
specified branch*, with the option to default to the main branch.
...
#+begin_src conf
    ...
    delete-remote-branch = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git delete-remote-branch BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
        BRANCH_NAME=\"$1\"; \
...
#+begin_src conf
    ...
    delete-branch = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git delete-branch BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
        BRANCH_NAME=\"$1\"; \
...
*** Remove orphaned branches (those that no longer exist remotely)

When a branch is deleted from a remote repository, Git keeps the corresponding
*remote-tracking branch* until it is pruned.

Prune remote-tracking branches that no longer exist on the default remote:

: git remote prune $(git origin)

Use ~--dry-run~ to preview which branches would be removed:

: git remote prune $(git origin) --dry-run

To prune *all* remotes:

#+begin_src conf
    ##? Usage: git prune-branches-all-remotes
    ##?
    ##? Prune remote-tracking branches for all configured remotes.
...
#+begin_tip
To prune remote-tracking branches automatically whenever you fetch or pull:

: git config --global fetch.prune true
#+end_tip

Pruning *only removes remote-tracking branches*. It does *not* delete your local
branches, even if their upstream branch has been removed.

To delete *local branches* whose configured upstream branch is marked as
~[gone]~:

#+begin_src conf
    ##? Usage: git delete-gone-local-branches
    ##?
    ##? Delete local branches whose configured upstream branch is gone.
    ##? Skip the currently checked-out branch.
    delete-gone-local-branches = "!f() { \
        ...
        if [ -n \"$gone_branches\" ]; then \
            ...
            printf '%s\n' 'No gone branches to delete.'; \
        fi; \
    }; f"
#+end_src

The cleanup is therefore a two-step process:

1. Prune remote-tracking branches (~git remote prune~ or ~fetch.prune=true~).
2. Delete local branches whose upstream is now reported as =[gone]=
   (~git delete-gone-local-branches~).
...
#+begin_src conf
    ...
    rename-branch = "!f() { \
        if [ $# -ne 2 ]; then \
            printf 'Usage: git rename-branch OLD_BRANCH_NAME NEW_BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
...
Display all tags in the repository, using /version-aware sorting/:
...
This ensures that tags like ~v0.10~ correctly follow ~v0.9~ (unlike simple
lexicographic sorting, which would sort ~v0.10~ before ~v0.9~).
...
:     | sed 's_\^{}__' \
:     | uniq -c \
:     | sed 's_2\ refs/tags/_ a _' \
:     | sed 's_1\ refs/tags/_lw _'
...
#+begin_src conf
    ...
    rename-tag = "!f() { \
        if [ $# -ne 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git rename-tag OLD_TAG NEW_TAG\n' >&2; \
            return 64; \
        fi; \
        OLD_TAG_NAME=\"$1\"; \
        NEW_TAG_NAME=\"$2\"; \
...
#+begin_src conf
    ##? Usage: git delete-tag TAG_NAME
    ##?
    ##? Delete a local and remote tag.
    delete-tag = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git delete-tag TAG\\n' >&2; \
            return 64; \
        fi; \
        TAG_NAME=\"$1\"; \
        git tag -d \"$TAG\" && \
...
Show the commits on the current branch that are not yet reachable from another
branch.

This uses Git's two-dot (~..~) revision range to list commits that are reachable
from ~HEAD~ but not from the specified branch. It answers the common question:
/"What commits have I made on my current branch that are not yet in ~BRANCH~?"/

#+begin_src conf
    ##? Usage: git ahead-of BRANCH
    ##?
    ##? Show commits on HEAD that are not reachable from BRANCH.
    ahead-of = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git ahead-of BRANCH\n' >&2; \
            return 64; \
        fi; \
        BRANCH=\"$1\"; \
        COUNT=$(git rev-list --count \"$BRANCH\"..HEAD) || return; \
        printf 'You have %d extra commit%s compared to %s:\n' \
            \"$COUNT\" \
            \"$( [ \"$COUNT\" -eq 1 ] || printf s )\" \
            \"$BRANCH\" >/dev/stderr; \
        git --no-pager log --decorate \"$BRANCH\"..HEAD; \
        printf '\n'; \
    }; f"
#+end_src
...
#+begin_src conf
    ...
    ##?
    ##? Show commits on BRANCH that are not reachable from HEAD.
    behind = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git behind BRANCH\n' >&2; \
            return 64; \
        fi; \
        BRANCH=\"$1\"; \
        COUNT=$(git rev-list --count HEAD..\"$BRANCH\") || return; \
        printf 'You are missing %d commit%s from %s:\n' \
            \"$COUNT\" \
            \"$( [ \"$COUNT\" -eq 1 ] || printf s )\" \
            \"$BRANCH\" >/dev/stderr; \
        git --no-pager log --decorate HEAD..\"$BRANCH\"; \
        printf '\n'; \
    }; f"
#+end_src
...
Show the files changed on the current branch since it diverged from another
branch.

This uses Git's three-dot (~...~) revision range, which compares ~HEAD~ against the
merge base with the specified branch. It answers the common question: /"What
files have changed on my current branch since it was created from ~BRANCH~?"/

#+begin_src conf
    ##? Usage: git changed-files-since BRANCH
    ##?
    ##? Show changed files on HEAD since it diverged from BRANCH.
    changed-files-since = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git changed-files-since BRANCH\n' >&2; \
            return 64; \
        fi; \
        BRANCH=\"$1\"; \
...
- Inline diff (unified, single view)
  ...
  #+begin_src conf
    ...
    compare = "!f() { \
        if [ $# -ne 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git compare FROM TO\n' >&2; \
            return 64; \
        fi; \
        FROM=\"$1\"; \
        TO=\"$2\"; \
  ...
  Compare a specific file between the main branch and the current branch:
...
: git diff BRANCH_NAME_1..BRANCH_NAME_2 -- PATH/TO/FILE
...
The ~missing~ alias (inspired by Bazaar) displays the commits that are unique to
the current branch and the target branch, making it easy to see what has not yet
been merged.

#+begin_src conf
    ##? Usage: git missing [BRANCH]
    ##?
    ##? Show commits that differ between HEAD and BRANCH.
    missing = "!f() { \
        if [ $# -gt 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git missing [BRANCH]\n' >&2; \
            return 64; \
        fi; \
        if [ $# -eq 1 ]; then \
            BRANCH=\"$1\"; \
        else \
            BRANCH=$(git upstream-branch) || return; \
...
To compare the current branch with its upstream branch:

: git missing

To compare the current branch with a specific branch:

: git missing main

The output consists of two sections:

- *Ahead of*: commits that exist only on the current branch.
- *Behind*: commits that exist only on the target branch.

*** Alternative Git commands

If you only want to list commits that exist on your current branch but not on
~main~, Git provides several built-in alternatives.

Assuming your branch was created from ~main~ and is currently checked out:
...
These commands list commits that have not yet been merged into ~main~. The
~git missing~ alias provides a more complete view by showing both commits unique
to the current branch and commits unique to the comparison branch.
...
changes on the current (=feature=) branch to upstream (=main=) and indicates which
...
The *most recent common ancestor* commit between two branches is the last commit
that existed in both branches. Usually this is the branch point, but it could
also be a commit that was merged.
...
#+begin_src conf
    ...
    common-ancestor = "!f() { \
        ...
        if [ -z \"$base\" ]; then \
            printf '%s\n' \"ERROR: Unable to find common ancestor between branches '$MAIN_BRANCH' and '$CURRENT_BRANCH'. Branches may not exist or be valid.\" >&2; \
            return 2; \
        fi; \
...
To show all the *changed files* (see the files that have been /added/, /copied/,
/deleted/, /modified/, or /renamed/) in a specific Git branch *since it was forked* from
another branch, run:

: git diff main...feature_branch --name-status --diff-filter=ACDMR

Available Filters for ~--diff-filter~:
- A :: Added
- C :: Copied
- D :: Deleted
- M :: Modified
...
#+begin_src conf
    ...
    missing-files = "!f() { \
        ...
        if ! git verify-local-branch \"$current_branch\"; then \
            printf \"ERROR: Current branch '%s' is not a valid branch.\n\" \"$current_branch\" >&2; \
            return 2; \
        fi; \
        if ! git verify-local-branch \"$OTHER_BRANCH\"; then \
            printf \"ERROR: Other branch '%s' is not a valid branch.\n\" \"$OTHER_BRANCH\" >&2; \
            return 2; \
        fi; \
...
See http://jonas.nitro.dk/tig/manual.html and
...
#+begin_src conf
    ##? Usage: git lg
    ##?
    ##? Show the commit history as a graph for the current revision set.
    ##? Decorations (branches and tags) are shown according to Git configuration.
    lg = log --graph --decorate

    ##? Usage: git lg-all-branches
    ##?
    ##? Show the commit history graph for all local branches, remote-tracking
    ##? branches, and tags.
    lg-all-branches = log --all --graph --decorate
#+end_src

Graph + changed files:

#+begin_src conf
    ##? Usage: git lg-changes
    ##?
    ##? Show a detailed commit history graph with relative commit dates,
    ##? authors, decorations, and the list of files changed in each commit.
    lg-changes = log --pretty=format:'%C(green)%cr%C(reset) %C(yellow)%h%Creset %C(yellow)%d%Creset %s %C(bold blue)<%an>%Creset%n' --graph --name-status
#+end_src

https://fossil-scm.org/home/help?cmd=leaves

Useful for visualizing releases, tags, branch tips, and other important
milestones in the repository history:

#+begin_src conf
    ##? Usage: git releases
    ##?
    ##? Show a simplified history graph containing only commits referenced by
    ##? branches or tags.
...
: #!/usr/bin/env bash
: TMP_FILE=$(mktemp)
: REPO_SLUG_REMOTE=$(git remote -v | grep '^upstream' | head -n 1 | perl -lne 's_github.com:?\/?(.*)\/([^.]*)([.]git| )__ or next; print $1,"/",$2')
: PR_NUMBER=$(git current-branch | cut -d/ -f 2)
: curl -s https://api.github.com/repos/$REPO_SLUG_REMOTE/pulls/$PR_NUMBER > $TMP_FILE
...
#+begin_src conf
    ...
    merge-pr = "!f() { \
        if [ $# -ne 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git merge-pr REMOTE PR_NUMBER\\n' >&2; \
            return 64; \
        fi; \
        USER_AND_REPO=$(git remote show \"$1\" | grep 'Fetch URL:' | sed -E 's_.*github.com[:\\/]([a-zA-Z_-]+)\\/([a-zA-Z_-]+)\\.git$_\\1 \\2_'); \
        if [ -z \"$USER_AND_REPO\" ]; then \
            printf 'ERROR: Could not determine GitHub user/repo from remote\\n' >&2; \
            return 2; \
        fi; \
        ...
        if ! grep -q '\"title\":' \"$TMP_FILE\"; then \
            printf 'ERROR: Failed to fetch PR #%s\\n' \"$2\" >&2; \
            rm -f \"$TMP_FILE\"; \
            return 2; \
        fi; \
        PULL_AUTHOR=$(awk '/\"head\":/{flag=1} flag&&/\"login\": \"/{print \\$2; exit}' \"$TMP_FILE\" | sed -E 's_\"([^\"]+)\".*_\\1_'); \
        PULL_REF=$(awk '/\"head\":/{flag=1} flag&&/\"ref\": \"/{print \\$2; exit}' \"$TMP_FILE\" | sed -E 's_\"([^\"]+)\".*_\\1_'); \
        PULL_TITLE=$(grep '\"title\": ' \"$TMP_FILE\" | sed 's_^ *\"title\": \"__' | sed 's_\",$__'); \
        rm -f \"$TMP_FILE\"; \
        if [ -z \"$PULL_AUTHOR\" ] || [ -z \"$PULL_REF\" ] || [ -z \"$PULL_TITLE\" ]; then \
            printf 'ERROR: Failed to parse PR details\\n' >&2; \
            return 2; \
        fi; \
...
#+begin_src conf
    ...
    release-tag = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git release-tag VERSION\n' >&2; \
            return 64; \
        fi; \
        VERSION=\"$1\"; \
...
#+begin_note
Although controversial for some projects, replacing manual =ChangeLog= files with
logs generated from commits has pros and cons.

*Important considerations:*
- How to mark commits that should be excluded from =ChangeLog=.
- Handling log corrections (e.g., via notes).
- Need for clear documentation on this process.
#+end_note
...
(From Catalin)
: git archive --format="zip" 05eb104f1 --output="C:\Users\catal\Downloads\stack-2_05eb104f1.zip" "file1" "file2" "file3" ... "fileN"
(with list of files modified between 2 commits)
...
#+begin_src conf
    ...
    export-changed-files = "!f() { \
        ...
        if [ -z \"$changed_files\" ]; then \
            printf 'ERROR: No changed files to export.\n' >&2; \
            return 2; \
        fi; \
        date=$(date +'%Y-%m-%d'); \
...
: git missing-files-name-only main > file-list.txt
: cat file-list.txt | xargs zip -9 "$(date +'%Y-%m-%d')_what-it-does.zip"
...
#+begin_src conf
    ##? Usage: git export-commit-files [COMMIT]
    ##?
    ##? Export changed files from a commit or HEAD and create a zip archive.
    export-commit-files = "!f() { \
        COMMIT=$(git rev-parse --short ${1:-HEAD}); \
        current_branch=$(git convert-current-branch-name); \
        zip_file_name=$(git repo-slug-remote)-commit-${current_branch}-${COMMIT}.zip; \
        git changed-files \"$COMMIT\" | tr '\n' '\\0' | xargs -0 zip -9 \"$zip_file_name\"; \
    }; f"
#+end_src

export-commit-files-files = "!f() {
    targe...
...
follow the format ~X.Y.Z~ (*without* the ~v~ prefix). This format ensures
compatibility with tools like gitweb or GitHub, which can automatically provide
a download link for a tarball or zip archive in the format
=package-$TAG.tar.gz= -- and it's widely accepted that tarballs should adhere to
the naming convention =package-X.Y.Z.tar.gz=.
...
#+begin_src conf :tangle no
    issues = "!f() { \
        USER_NAME=\"$1\"; \
        curl --user $USER_NAME 'https://api.bitbucket.org/2.0/repositories/$(git repo-owner)/$(git repo-slug-remote)/issues?pagelen=100' \
            | jq --compact-output '.values | .[] | [.id, .title, .type, .priority, .state, .votes, .assignee.display_name, .created_on, .updated_on]' \
            | sed \"s_\"open\"_$(tput setaf 2)&$(tput sgr0)_\" \
            | sed \"s_\"closed\"_$(tput setaf 1)&$(tput sgr0)_\"; \
    }; f"
#+end_src
...
: curl -u $USER_NAME -X GET 'https://api.bitbucket.org/2.0/repositories/$(git repo-owner)/$(git repo-slug-remote)/issues?pagelen=100' \
:     | jq --compact-output '.values | .[] | [.id, .title, .type, .priority, .state, .votes, .assignee.display_name, .created_on, .updated_on]'
: # curl -s -S
...
#+begin_src conf :tangle no
    issues-created-by-me = "!f() { \
        ...
        API_URL=\"https://api.github.com/repos/$REPO_OWNER/$REPO_SLUG_REMOTE/issues?creator=$REPO_OWNER\"; \
...
#+begin_src conf
    ##? Usage: git download-repository [COMMIT]
    ##?
    ...
    download-repository = "!f() { \
        COMMIT=${1:-HEAD}; \
        git archive $COMMIT --format=zip --output=$(basename $(git root))-commit-$(git rev-parse $COMMIT).zip; \
    }; f"
#+end_src
...
so that you can do things like ~git x make~.  This works because Shell aliases are
...
#+begin_src conf
    ...
    all-repos = "!f() { \
        if [ $# -lt 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git all-repos COMMAND [ARGS]\\n' >&2; \
            return 64; \
        fi; \
        find . -type d -name '.git' -prune 2>/dev/null \
            ...
            | while read -r repo_path; do \
                if ! repo_slug_local=$(git -C \"$repo_path\" repo-slug-local 2>/dev/null); then \
                    repo_slug_local=\"$repo_path\"; \
                fi; \
...
*NOTE --* Some commands, like ~st~, are displayed in colors. Others, like ~branches~
are not...

*** Display a summarized version of the status of each repository
...
uncommitted work, I rely on

: git all-repos status --short

to rapidly assess repositories that contain pending changes.

*** Display the remote repository URLs for each repository

: git all-repos remote -v

*** Find all "unpushed" commits for all projects in a directory
...
XXX cherry -v much quicker...

*** Display branch information for each repository

: git all-repos br

*** Pull new changes / push any local changes of each repository
...
#+begin_src conf
    ...
    # git remote show -n origin | grep "Fetch URL" | cut -d: -f2- | sed 's_^ *__'
#+end_src
...
#+begin_tip
Beginning with Git version 1.7.0, ~@{u}~ is a convenient shorthand notation for
referring to the *upstream* branch of the *current* branch. For instance, if you are
on the ~foo~ branch, the upstream branch would be ~origin/foo~ (or ~github/foo~, etc.,
depending on the remote's name), and you can use ~@{u}~ to represent it.
#+end_tip
...
#+begin_src conf
    ...
    merge-into-main = "!f() { \
        ...
        if git merge \"$current_branch\"; then \
            ...
            printf \"\\033[1mUnable to merge branch:\\033[0m\n\" >&2; \
            return 2; \
        fi; \
        git checkout \"$current_branch\"; \
    }; f"
...
- *Branch merged:* 'branch_name' successfully merged
...
If you now type ~git merge-update-push staging~ while on the ~feature~ branch, this
will happen:
...
Run ~merge-check~ (before ~merge~) to check for any conflicts beforehand.

#+begin_src conf
    ##? Usage: git merge-check BRANCH_TO_MERGE
    ##?
    ##? Check if a branch merges without conflicts; leaves no changes behind.
    ##?
    ##? Example:
    ##?   git merge-check feature
    merge-check = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git merge-check BRANCH_TO_MERGE\n' >&2; \
            return 64; \
        fi; \
        BRANCH_TO_MERGE=\"$1\"; \
        ...
        if [ $status -eq 0 ]; then \
            printf 'Merge would work.\n'; \
        else \
            printf 'Merge has conflicts or failed.\n'; \
        fi; \
        return $status; \
    }; f"
#+end_src
...
#+begin_src conf
    # Usage: git merge-tree-conflicts BASE_BRANCH FEATURE_BRANCH
    #
    # Show merge conflicts predicted by git merge-tree, without performing a merge.
    merge-tree-conflicts = "!f() { \
        if [ $# -ne 2 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git merge-tree-conflicts BASE_BRANCH FEATURE_BRANCH\n' >&2; \
            return 64; \
        fi; \
        BASE_BRANCH=\"$1\"; \
        FEATURE_BRANCH=\"$2\"; \
        MERGE_BASE=$(git merge-base \"$BASE_BRANCH\" \"$FEATURE_BRANCH\"); \
        git merge-tree \"$MERGE_BASE\" \"$BASE_BRANCH\" \"$FEATURE_BRANCH\" \
...
This command lists files that have merge conflicts (~--diff-filter=U~ filters for
*"unmerged" files*), that is files having leftover conflict markers after a merge
failure:
...
: feature-start = '!git checkout main; git pull; git create-branch "$1" main'

Update the feature branch:

: feature-pull = '!git checkout main; git pull; git checkout HEAD; git rebase main'

Share the feature branch:

: feature-push = '!git push -u $(git origin) HEAD'
...
*** If you get the error "Your local changes to the following files would be overwritten by checkout"
...
To address the case where local changes to tracked files would be overwritten by
the checkout operation, you have a few options:
...
- *Commit the Changes:* If the changes are ready to be committed, you can commit
  them to your current branch before switching to the desired branch.
...
- *Stash the Changes:* If you're not ready to commit the changes but want to
  switch branches, you can stash them using ~git stash~. Stashing temporarily
  shelves changes so you can reapply them later.
...
- *Discard the Changes:* If the changes are not needed or you want to discard
  them, you can use ~git checkout -- FILE~ (*unmodify*) to discard changes for
  individual files

  #+begin_src shell :tangle no
  # Attempt to switch to the desired branch, while handling errors.
  git checkout BRANCH_NAME 2>&1 | {
      # Read each line of output.
      while IFS= read -r file_path; do
          # Remove leading spaces from file_path.
          file_path=$(printf '%s\n' \"$line\" | sed 's_^[[:space:]]*__')
          # If a file path is found, discard changes for that file.
          if [ -n "$file_path" ]; then
  ...
  #+begin_src conf
    ...
    force-checkout = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: git force-checkout BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
        BRANCH_NAME=$1; \
        git checkout \"$BRANCH_NAME\" 2>&1 | { \
  ...
  or ~git checkout -- .~ to discard changes for all tracked files in the working
  directory.
...
Stash local changes, checkout branch, and reapply stashed changes.
...
#+begin_src conf
    ...
    co-autostash-include-untracked = "!f() { \
        if [ $# -ne 1 ] || [ \"$1\" = \"-h\" ] || [ \"$1\" = \"--help\" ]; then \
            printf 'Usage: co-autostash BRANCH_NAME\n' >&2; \
            return 64; \
        fi; \
        BRANCH_NAME=\"$1\"; \
...
# Attempt to switch to the desired branch, while handling errors.
git checkout BRANCH_NAME 2>&1 | while read -r file_path; do
    # Check if the file exists to avoid errors.
    if [ -e "$file_path" ]; then
        # Move the untracked file to a backup location.
...
#+begin_src shell :tangle no
printf 'setting up gh-pages\n'
printf '-------------------\n'

printf 'Tell me your github account username: '
read USER_NAME

printf 'Now, tell me your repository name: '
read REPOSITORY
...
git stash \
    ...
    && printf 'Complete\n' \
    && printf '-------------------\n' \
    && printf 'You can find your last changes in the stash!\n'
#+end_src
...
2. Then, you can just ~git create-branch BRANCH_NAME SHA_1~ to recreate the branch from
   there.
...
#+begin_note
Before pushing your changes to a remote repository, it's wise to review your
modifications in advance. This proactive step helps streamline the process of
rectifying errors, should any arise.

To do that, use the command "git diff origin/master". If you want to show your
unpushed commits with their commit log messages, use "git show origin/master.."
instead. If you only have one local commit you want to push, just "git show" is
enough.
...
The ~outgoing~ command returns a list of *unpushed* commits for the *current branch*,
the next time you do a push.
...
*************** TODO Fix missing newline at the end... Otherwise, wrong count of lines (with shell alias "W")...
...
The command ~compare-remote-branch-log~ is useful for quickly showing how a local
branch and its remote version have *diverged*. It helps determine which commits
are unique to the local branch (see also ~git out~) and which exist only on the
remote (see also ~git in~). This can be helpful before merging, rebasing, or
pushing changes.
...
The starting point is essential because it defines the commit from which you
want to generate the patch files...
...
#+begin_src shell :tangle no
# Download a patch from a URL in the clipboard and verify it.
alias git-patch-verify='curl "$(pbpaste)" | git apply --check --verbose'

# Download a patch from a URL in the clipboard and apply it.
alias git-patch-apply='curl "$(pbpaste)" | git apply --verbose'

# Download a patch from a URL in the clipboard, apply it, and stage the changes.
alias git-patch-apply-stage='curl "$(pbpaste)" | git apply --verbose --index'
#+end_src
...
Just insert ~trace~ between ~git~ and whatever would usually follow; for example:
...
This alias helps identify *dangling commits* -- those that have been deleted or
are no longer reachable from any current branch. It's particularly useful if
branches have been deleted or commits have become orphaned due to operations
like rebases.
...
By using this alias, you can *locate* these commits and *recover* any important
*changes* that might otherwise be lost.
...
Your =~/.gitconfig= file might look like the following:

#+begin_src conf :tangle no
[include]
    path = .gitconfig_gitboost

[user]
    name = Your Name
    email = home.email@example.com

# Include common configuration for client projects.
[includeIf "gitdir:~/1-Clients/"]
    path = ~/1-Clients/.gitconfig
#+end_src

** Splitting configuration across files

Git supports splitting configuration across multiple files. This is particularly
useful for keeping personal or sensitive settings separate from your main
configuration:
...
This allows you to organize your Git configuration across several files. For
example, you can keep general settings in =~/.gitconfig= and store private
information, such as GitHub credentials, access tokens, or work-specific
settings, in a separate file such as =~/.githubconfig=.

This approach keeps your main configuration clean and shareable while reducing
the risk of accidentally exposing sensitive data.

** Conditional configuration

Git can also load configuration files conditionally based on the location of
a repository.

In the example above, /all repositories located under/ =~/1-Clients/= automatically
include the configuration stored in =~/1-Clients/.gitconfig=:
...
This makes it easy to apply a common configuration to a group of related
repositories without duplicating settings.

** Overriding settings for work projects

If you need to use a different email address for client or work projects, create
a =.gitconfig= file in the corresponding directory:
...
The settings from this file will be applied automatically to repositories
covered by the matching ~includeIf~ rule.
...
[[https://github.com/fniessen/gitboost/issues/new][GitHub issue tracker]].
...
If you find the "gitboost project" (or any of
[[https://github.com/fniessen/][my other projects]]) enhancing your Git experience and simplifying your workflow,
seize the opportunity to express your appreciation!  Help fuel future
development by making a [[https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=VCVAS6KPDQ4JC&lc=BE&item_number=gitboost&currency_code=EUR&bn=PP%2dDonationsBF%3abtn_donate_LG%2egif%3aNonHosted][donation]] through PayPal. Your support is invaluable --
thank you!

Remember, regardless of donations, "gitboost project" will
always remain freely accessible, both as in Belgian beer and as in speech.
...
#+html: </a>
#+html: <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=VCVAS6KPDQ4JC&lc=BE&item_number=gitboost&currency_code=EUR&bn=PP%2dDonationsBF%3abtn_donate_LG%2egif%3aNonHosted">
