...
#+begin_src conf
    ...
    # Usage: git grep-all2 SEARCH_TERM
    #
    # Search for a specific string in all Git-tracked files within the
    # repository. This alias is useful for quickly finding occurrences of
    # a particular term across the entire codebase.
    grep-all2 = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git grep-all SEARCH_TERM\n'; \
            return 2; \
        fi; \
        git grep -n -- \"$1\"; \
    }; f"
