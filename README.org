...
Welcome to gitboost!  This repository contains
a collection of configuration settings and aliases to enhance your Git workflow.
...
contents, try them out, and feel free to contribute your own enhancements.
Let's make Git even better together!
...
if ! command -v zip > /dev/null 2>&1; then
    printf >&2 "WARNING: 'zip' is not installed. Please install it.\n"
fi
#+end_src
...
#+begin_src conf
    ...
    clean-only-untracked = "!f() { \
      printf 'WARNING: This will permanently delete all untracked files and directories.\\n'; \
...
#+begin_src conf
    ...
    clean-only-ignored = "!f() { \
      printf 'WARNING: This will permanently delete ignored files and directories.\\n'; \
      printf 'Are you sure? [y/N] '; \
      read -r confirm; \
      if [ \"$confirm\" = y ] || [ \"$confirm\" = Y ]; then \
        git clean -f -d -X; \
...
#+begin_src conf
    ...
    ##? THIS COMMAND MAY DELETE FILES THAT CANNOT BE RECOVERED.
    clean-all = "!f() { \
      printf 'WARNING: This will permanently delete *all* ignored and untracked files and directories!\\n'; \
