...
#+begin_src conf
    ...
    ##?
    ##? Show a graphical log of all references with branches, and tags.
    lg-all-branches = log --all --graph

    ##? Usage: git lg-detail
    ##?
    ##? Show a detailed graphical log with file changes.
    lg-detail = log --pretty=format:'%C(green)%cr%C(reset) %C(yellow)%h%Creset %C(yellow)%d%Creset %s %C(bold blue)<%an>%Creset%n' --graph --name-status
#+end_src
